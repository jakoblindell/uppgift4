...
class ImageClassificationModel {
    ...
    func classify(choosenImage: String) -> MobileNetV2Output {
        
        let defaultConfig = MLModelConfiguration()
        let imageClassifierWrapper = try? MobileNetV2.init(configuration: defaultConfig)
        ...
        guard let output = try? imageClassifier.prediction(image: imageBuffer) else {
            fatalError("Could not get imageBuffer")
        }
